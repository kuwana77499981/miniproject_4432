...
//header('Content-Type: application/json; charset=UTF-8');
    ...
    function getTag(){
        ...
        $query = "select TagName ,Qty from Book,BookTag,Tag where BookTag.TagID=Tag.TagID && Book.BookID=BookTag.BookID && Book.BookID=\"".$bookid."\" order by Qty DESC limit 10";
        ...
        if (!$result)  die ("cannot open sql file");
            ...
            while ($row = $result->fetch_assoc())  {
                ...
                $msg.="<button id=\"".$row["TagName"]."\" value=\"".$row["TagName"]."\" class=\"tagButton sb\" style=\"color:black;background:rgb(255," . (255-$colorDepth).",". (255-$colorDepth).")\">".$row["TagName"]." (".$row["Qty"].")</button> ";
            }
        }
    ...
    function insertTag(){
        ...
        $query = "select TagName,BookTag.BookID,Qty from Tag,BookTag,Book where TagName=\"".$tag."\" && Book.BookID= \"".$bookid."\"&& Book.BookID=BookTag.BookID && Tag.TagID=BookTag.TagID";
        $result = $conn->query($query);
        $result->data_seek(0);
        ...
        if (!$row["TagName"]) {
            ...
            if(!($conn->query($sql) === TRUE)){
                ...
                /*TODO:change color*/$msg="<button id=\"".$tag."\" value=\"".$tag."\" class=\"sb tagButton\" style=\"color:black;background:rgb(255,255,255)\">".$tag." (1)</button> <button id=\"plus\" class=\"sb tagButton\" value=\"addANewTag\"><b>+</b></button>";
                echo json_encode(array( 'action'=>$action,'msg'=>$msg));
            }
        }
        else {
            ...
            if ($conn->query($sql) === TRUE) {
                ...
                $msg="<button id=\"".$row["TagName"]."\" value=\"".$row["TagName"]."\" class=\"sb tagButton\" style=\"color:black;background:rgb(255," . (255-$colorDepth).",". (255-$colorDepth).")\">".$row["TagName"]." (".$qty.")</button> ";
                echo json_encode(array( 'action'=>$action,'msg'=>$msg));
            } else {
